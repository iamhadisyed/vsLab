...
function opengrading(userid,taskid,email,taskname,groupid) {
    // if (taskid=='101'){
    ...
    // }else{
        var subtaskarray = [1, 2, 3, 4];
        var message ='<h2>Task: '+ taskname +'</h2>';
        var report=[];
        subtaskarray.forEach(function(subtaskid){
            ...
            if(data[i].type==1) {
                ...
                var taskcontent = '<div class="subtasksubmission">' +
                    ...
                    // '<input type="text" id="screenshottitle" maxlength="10"/><br/>
                     'Feedback:<textarea spellcheck="true" style="max-width: 100%;width : 100%" class="gradingfeedback" id="'+ data[i].id+'" cols="50" rows="2">'+data[i].feedback+'</textarea>' +
                    '</p></div>';
                ...
                var taskcontent = '<div class="subtasksubmission">' +
                    '<p>Answer: <font color="blue">' + data[i].desc +
                    '</font><br/>' +
                    'Feedback:<textarea spellcheck="true" style="max-width: 100%;width : 100%" class="gradingfeedback" id="'+ data[i].id+'"  cols="50" rows="2">'+data[i].feedback+'</textarea>' +
                    '</p></div>';
                report[data[i].subtask_id] = report[data[i].subtask_id] + taskcontent;
    ...
    $.getJSON("/getgrade", {
        ...
        if(data.graded==1) {
            ...
            $('#givenpoints').val('No grade');
        }
    });

}

function gradingpolicy(tasks,groupid,labid) {
    var labname= $('#labselector option:selected')[0].text

    if(labid==null||labid=='0'){
        alert('Please select a lab first.')
    }else if(tasks.length==0){
        alert('No task in selected lab!')
    }else{
        var ask = labname + ' Grading Policy';
        var message = '<p>Total points for ' + labname +
            ': <spam id="totalpoints" /></p>';
        for(var i=0;i<tasks.length;i++){
            message =message +'<p>Points for '+tasks[i].name+'</br>' +
                '<input type="text" id="taskpoint'+tasks[i].id+'" maxlength="5"  required /></p>';
        }


        create_ConfirmDialog('.container-fluid', ask, message, '20%', '.container-fluid',
            function () {
                var labpoint=0
                for(var i=0;i<tasks.length;i++){
                    tasks[i].fullgrade=$('#taskpoint'+tasks[i].id).val();
                    labpoint= Number(labpoint)+Number(tasks[i].fullgrade);
                }
                $.post("/gradingpolicyforlab", {
                        "labid": labid,
                        "labpoint": labpoint,
                        "groupid":groupid,
                        "tasks": tasks
        ...
        $.getJSON("/getlabgradingpolicyforlab", {

            "labid": labid,
            "groupid":groupid,

        }, function (data) {
            $('#totalpoints').html(data.labfullpoints);
            for(var i=0;i<tasks.length;i++){
                $('#taskpoint'+tasks[i].id).val(data.tasksfullpoints[tasks[i].id][0]);
...
function submitgrade(){
    var feedback = $('#taskgradingfeedback').val();
    var taskid=$('#gradingtaskid').html();
    var grade = $('#givenpoints').val();
    var userid =$('#gradinguserid').html();
    var submissionfeedback={};
    $('.gradingfeedback').each(function() {
        submissionfeedback[this.id]=this.value;
    });
    ...
    $.post("/gradetask", {
        ...
        function (data) {
            $("tr td:contains("+$('#gradingusername').html()+")").each(function(){
                if($(this).next('td').next('td').html()==data[0]){
                    $(this).next('td').next('td').next('td').html(grade);
                }
...
function submitgrade(i){
    if(i == null){
        ...
        var feedback = $('#taskgradingfeedback'+i).val();
        var taskid=$('#gradingtaskid'+i).html();
        var grade = $('#givenpoints'+i).val();
        var userid =$('#gradinguserid'+i).html();
        var submissionfeedback={};
        $('.gradingfeedback'+i).each(function() {
            submissionfeedback[this.id]=this.value;
        });
    }
    ...
    $.post("/gradetask", {
        ...
        function (data) {
            $("tr td:contains("+$('#gradingusername'+i).html()+")").each(function(){
                    $(this).next('td').next('td').html(
                        $(this).next('td').next('td').html()-data[1]+Number(grade)
                    );
            });
            alert('Grading Saved!');
...
function gotolabgrading(element) {
    var tr = $(element).closest('tr');
    var row = $('#dataTableBuilder').DataTable().row(tr).data();
    var labid = row.id;
    var url = window.location.href.split( '/' );
    var groupid= url[ url.length - 2 ];
    if (groupid=='697'){
        window.location.href = '/grade/'+groupid+'/695';
    }else{
        window.location.href = '/grade/'+groupid+'/0';
    }

}

function openlabgrading(userid,labid,email,groupid) {
    // var tr = $(element).closest('tr');
    // var row = $('#dataTableBuilder').DataTable().row(tr).data();
    // var labid = row.id;
    // var groupid=window.location.href.substring(window.location.href.lastIndexOf('/') + 1).replace('#','');
    $.getJSON("/getlabcontentbylabid", {
        ...
        for(i=0;i<taskids.length;i++){
            openlabtaskgrading(userid,taskids[i][0],email,groupid,taskids[i][1],i,taskids.length);
        }
        // taskids.forEach(function (item) {
        //     openlabtaskgrading(userid,item[0],email,groupid);
        // })
    });
}

function openlabtaskgrading(userid,taskid,email,groupid,taskname,j,l) {

    var subtaskarray = [1, 2, 3, 4];
    var k=j+1;
    var message ='<h2>Task '+ k + ': '+ taskname +'</h2>';
    var report=[];
    subtaskarray.forEach(function(subtaskid){
        report[subtaskid]='<div class="report-subtask'+subtaskid+'"';
    });
    ...
    $.getJSON("/tasksubmissionbyuser", {
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            if(data[i].type==1) {
                ...
                var taskcontent = '<div class="subtasksubmission">' +
                    ...
                    '</font><br/>' +
                    // '<input type="text" id="screenshottitle" maxlength="10"/><br/>
                    'Feedback:<textarea spellcheck="true" style="max-width: 100%;width : 100%" class="gradingfeedback'+j+'" id="'+ data[i].id+'" cols="50" rows="2">'+data[i].feedback+'</textarea>' +
                    '</p></div>';
                report[data[i].subtask_id] = report[data[i].subtask_id] + taskcontent;
        ...
        $('#tobegrading'+j).html(message).css('height','750px').css('overflow-y','auto');


        $('.content-wrapper').height(l*1000);
        $('#gradingtaskid'+j).html(taskid);
        $('#gradinguserid'+j).html(userid);
        $('#gradingusername'+j).html(email);
        $('#taskname'+j).html(taskname);
        $('#tasktitle'+j).html('Submission Grading for Task '+k);
        if((imagecount==0)&&(filecount==0)&&(textcount==0)){

        }else{
            $('#gradingbox'+j).show();
            $('#submissionbox'+j).show();
        }
        // if(filecount==0){
        //     $('#filearea').empty();
        // }
    ...
    $.getJSON("/gettaskfullgrade", {
        ...
        $('#totalpointoftask'+j).html(data.fullgrade);
    ...
    $.getJSON("/getgrade", {
        ...
        $('#taskgradingfeedback'+j).val(data.feedback);

        if(data.graded==1) {
            $('#givenpoints'+j).val(data.grade);
        }else{
            $('#givenpoints'+j).val('No grade');
        }
    });
