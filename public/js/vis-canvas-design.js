...
function edit_lab_env(element){
    var tr = $(element).closest('tr');
    var row = $('#dataTableBuilder').DataTable().row(tr).data();
    var labid = row.id;
    window.location = '/labenvdesign/'+labid;
}

function startlab(element){
    var tr = $(element).closest('tr');
    var row = $('#dataTableBuilder').DataTable().row(tr).data();
    var labid = row.id;
    window.location = '/labenv/'+labid;
}


function save_design_to_DB(element) {
    var lab_design_json = {nodes: lab_design_data['new'].nodes.get(), edges: lab_design_data['new'].edges.get()};
    ...
    if ($("input[name=temp_name]").val() == '' || $("textarea[id=save_design_des_text]").val() == '') {
        ...
        $.post("labenv/saveTemplate", {
                "temp_name": $("input[name=temp_name]").val(),
                "temp": JSON.stringify(lab_template),
...
function delete_design_from_DB(element) {
    var lab_design_json = {nodes: lab_design_data['new'].nodes.get(), edges: lab_design_data['new'].edges.get()};
    var lab_design_vmcount = vmcount['new'];
    var lab_template = { "AWSTemplateFormatVersion": "2010-09-09", "Resources": lab_resources['new'] };
    if ($("input[name=temp_name]").val() == '' || $("textarea[id=save_design_des_text]").val() == '') {
...
function savelabenvdesign(element){
    var eid = element.id;
    ...
    if(proj_name == 'new') {
        ...
        var dlg_form = $(document.createElement('div')).appendTo($('#labenv'));
        ...
        var form_html = '<p>Template Name:</p>' +
            ...
            '<br><br>' +
            '<button id="save_design_to_DB" onclick="save_design_to_DB($(this))">Save</button>';
...
function vis_canvas_new_network_topology() {
    var project=0;
    ...
    // $('#btn-vis-refresh').css('background-color', '').attr('disabled', 'disabled');
...
function prepare_new_lab_design(tab_div,labid,labname) {
    var project = labid;
    var projectname = labname;
    ...
    '<tr><td>Type</td><td><select id="node-type_' + project + '" onchange="select_change(this);">' + '<option value="0">Select a Resource...</option>' +
    ...
    if (project != "new") {
        $('<span id="temp_name">Template : ' + projectname + '</span>').css('float', 'left').css('margin-left', '30%').appendTo(table_head);
    }
    ...
    if (project == "new") {
        newbtn2.addClass("dialog-btn").attr('id', 'btn_save_design_' + project).attr('onClick','savelabenvdesign(this)').css('float', 'right').text('Save Design');
    } else {
        newbtn2.addClass("dialog-btn").attr('id', 'btn_save_design_' + project).attr('onClick','savelabenvdesign(this)').css('float', 'right').text('Update Design');
    }
    ...
    mynetwork_div.attr('id', 'lab_design_div_' + project).css('position', 'relative')
        .css('width', '100%').css('height', 600).css('border', '1px solid lightgray');
...
// function prepare_lab_design(tab_div,labid,labname) {
//     var project = labid;
//     var projectname = labname;
//     var popup_div = $(document.createElement('div')).appendTo($(tab_div + project));
//     popup_div.addClass('addNode-popUp').attr('id', 'network-popUp_' + project)
...
//             else
//                 {
//                     alert('You may only have 10 vm nodes.');
//                     return;
//             }
//         }
//         else if (node_type == 'switch') {
...
//                     vmcount[project] = vmcount[project] + 1;
//             }
//                 else{ alert('You may only have 10 vm nodes.');
//                     return;
//                 }
//
//             }
...
//     lab_design_data[project] = {nodes: nodes, edges: edges};
//
//     var DIR = 'https://www.thothlab.com/workspace-assets/images/icons/';
//     var LENGTH_MAIN = 150;
//     var LENGTH_SUB = 50;
//     var temp = {};
//     var design = {};
...
//     var container = document.getElementById('lab_design_div_' + project);
//     var canvas_height = parseInt($('#lab_design_div_' + project).closest('div.window_inner').css('height'));
//     container.style.height = canvas_height - 50 + 'px';
//     container.style.minHeight = '600px';
//     var options = {
//         interaction: {
//             navigationButtons: true,
...
function new_lab_design(labid) {
    ...
    if (project !== 'new') {
        $.getJSON("/cloud/getDesign/" + project, function (jsondata) {
            ...
            design = JSON.parse(jsondata.temp_vis);
            lab_resources[project] = {};
            jQuery.extend(lab_resources[project], temp.Resources);
            nodes.add(design.nodes);
            edges.add(design.edges);
            for (vm in lab_resources[project]) {
                if (lab_resources[project][vm].Type == "OS::Neutron::Router") {
    ...
    function saveData(data, callback) {
        ...
        if (node_type == 'vm') {
            if (vmcount[project] <= 40){
                ...
                switch (os_type) {
                    case 'Quagga':
                        icon = "https://www.thothlab.com/workspace-assets/images/icons/network_router.png";

                        break;
                    case 'Ubuntu':

                        icon = "https://www.thothlab.com/workspace-assets/images/icons/terminal-ubuntu.png";
                    ...
                    case 'Windows':
                        ...
                        icon = "https://www.thothlab.com/workspace-assets/images/icons/terminal-windows.png";
                    ...
                    case 'Redhat':
                        ...
                        icon = "https://www.thothlab.com/workspace-assets/images/icons/terminal-redhat.png";


                        break;
                    case 'CentOS':

                        icon = "https://www.thothlab.com/workspace-assets/images/icons/terminal-centos.png";

                        break;
                    case 'Fedora':

                        icon = "https://www.thothlab.com/workspace-assets/images/icons/terminal-fedora.png";

                        break;
                    case 'Debian':

                        icon = "https://www.thothlab.com/workspace-assets/images/icons/terminal-debian.png";

                        break;
                    case 'Suse':

                        icon = "https://www.thothlab.com/workspace-assets/images/icons/terminal-suse.png";

                        break;
                    case 'NetBSD':

                        icon = "https://www.thothlab.com/workspace-assets/images/icons/terminal-netBSD.png";

                        break;
                    case 'OpenBSD':

                        icon = "https://www.thothlab.com/workspace-assets/images/icons/terminal-openBSD.png";


                        break;
                    case 'Metasploitable':

                        icon = "https://www.thothlab.com/workspace-assets/images/icons/terminal-Meta.png";


                        break;
                    case 'Kali (15.00 GB)':

                        icon = "https://www.thothlab.com/workspace-assets/images/icons/terminal-kali.png";
                    ...
                    default:
                        ...
                        icon = "https://www.thothlab.com/workspace-assets/images/icons/network_terminal.png";
...
function populate_lab_design(labid,labname) {

    prepare_new_lab_design('#lab_design',labid,labname);
    setTimeout(function () {
        new_lab_design(labid);
    }, 500);

}

function populate_new_lab_design() {




    prepare_new_lab_design('#lab_design','new','new');
    setTimeout(function () {
        new_lab_design('new');
    }, 500);
    ...
    $.post("/cloud/updateTemplate", {
        ...
        function (data) {
                ...
                if (data.status == "Success") {
                    ...
                    Swal.fire('Lab did mpt save!', '', 'fail');
                }
...
function deploy_lab_in_labenv(element) {
    var projects = [];
    ...
    var proj_desc = 'Test env project for labenv: ';
    ...
    for (var i = 0; i < labs.length; i++) {
        ...
        if (PROJECT_INVALID.indexOf(rowData.status) >= 0) {
            ...
            projects.push({ name: rowData.project_name, desc: proj_desc+ rowData.name + ', user: ' + rowData.ownerid, lab: rowData.id });
        }
    }
    ...
    if (invalid_projects.length > 0 || inprogress_projects.length > 0 || deployed_projects.length > 0) {
        ...
        Swal.fire({
            title: 'Deploy the selected lab environment?',
                ...
                projects.forEach(function(project, index) {
                    setTimeout(function() {
                        ...
                        $.post('labenv/deploy', {
                            ...
                            update_a_lab_status_in_labenv(row);
...
function update_a_lab_status_in_labenv(row) {
    var table = $('#dataTableBuilder').DataTable();
...
function update_labs_status_in_labenv() {
    var table = $('#dataTableBuilder').DataTable();
...
function modal_view_deployed_env(element) {
...
function deletelabenv(element){
    var tr = $(element).closest('tr');
    var row = $('#dataTableBuilder').DataTable().row(tr);
    var labenvid = row.data().id;
    Swal.fire({
        title: 'Delete this lab environment?',
        ...
        if (result.value) {
            $.post("/labenv/deleteTemp", {
                ...
                function (data) {
                    ...
                    //alert(data);
                    if (data.status == 'Success') {
                        Swal.fire('Lab Environment Deleted!', '', 'success');
...
function delete_test_env(element) {
    var table = $('#dataTableBuilder').DataTable();
    var PROJECT_DEPLOYED = ['CREATE_COMPLETE'];
    var PROJECT_IN_PROGRESS = ['CREATE_IN_PROGRESS', 'DELETE_IN_PROGRESS'];
    var deployed_projects = "";
    ...
    for (var i = 0; i < labs.length; i++) {
        ...
        if (PROJECT_DEPLOYED.indexOf(rowData.status) >= 0) {
            ...
            projects.push({id: rowData.project_id, lab: rowData.lab_id});
        }
    }
    if (deployed_projects.length > 0 || inprogress_projects.length > 0) {
        Swal.fire({
            title: 'Delete Lab Environment Error!', type: 'error',
            html: '<em>Reason</em><br />' +
        ...
        Swal.fire({
            title: 'Delete the lab environment?',
            text: 'It can\'t be recovered!',
            type: 'warning',
            showCancelButton: true,
            confirmButton: 'Yes'
                    ...
                    function (data) {
                        ...
                        if (data.status == 'Success') {
                            ...
                            Swal.fire({
                                title: 'Delete Lab Environment Error!', type: 'error',
                                html: '<em>Reason</em><br />' +
                                'This Lab Environment is still being used. Please unassign it from Group <b>'+data.groupname+'</b> in Lab Management!<br />'
...
function release_test_env(element) {
    var projects = [];
    var PROJECT_INVALID = ['PROJECT_FAILED'];
